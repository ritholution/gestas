...
class Gestas extends Base {
	...
	public function run() {
		switch ($this->configs['db_engine']) {
			case 'mysql':
				$this->db = new Mysql_db($this->configs['db_host'],
										 $this->configs['db_port'],
										 $this->configs['db_user'],
										 $this->configs['db_password'],
										 $this->configs['db_name']);
				break;
			default:
				throw new GDatabaseException(GDatabaseException::$DB_TYPE,
											 $this->configs['db_engine']);
				break;
		}
...
/*       $this->log = $_SESSION['log']; */
	...
	public function save_configurations() {
		...
		// it's inserted in the database, it's update. Elsewhere the configuration
		// it's inserted.
			...
			foreach ($this->config[$plugin] as $key => $value) {
				...
				if ($this->db->numRows() == 0) {
					...
					if ($row['valorConfiguracion'] != $value) {
						if (!$this->db->execute('update configuration set confValue=\'' . $value .
												'\' where idPlugin=' . $plugin . ' and confAttrib=\'' .
												$key . '\''))
							throw new GDatabaseException(GDatabaseException::$DB_CONSULT);
					}
				} else {
